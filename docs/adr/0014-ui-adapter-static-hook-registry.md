...
1. **Encapsulated Provider Hooks**: Each cloud provider is implemented in a dedicated sub-folder within `apps/webapp/src/adapters/` (e.g., `google/useGoogleDriveProvider.tsx`). This hook encapsulates all third-party React dependencies and returns a factory that implements standard `connect` and `restore` methods.
2. **Compile-Time Registry**: The file `apps/webapp/src/adapters/index.ts` exposes a `useCloudSyncProviderRegistry()` hook that synchronously composes and returns all registered adapter factories. 
...
- **Pros**:
  - Full decoupling: `AppContext` only interacts with abstract `CloudProviderFactory` objects.
